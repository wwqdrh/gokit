...
func TestSetupLocalDns(t *testing.T) {
	...
	forwardedPodPort := GetRandomTcpPort()

	if err := SetupLocalDns(forwardedPodPort, dnsPort, getDnsOrder("localDNS"), 10); err != nil {
		t.Error(err)
	}
	SetNameServer(fmt.Sprintf("%s:%d", "127.0.0.1", dnsPort), "localDNS")

	GetDnsRR("code-server")
}
